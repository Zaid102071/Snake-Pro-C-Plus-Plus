...
static struct {
    DWORD mode;
    bool saved;
} g_consoleState = {0, false};

void InputHandler::setupTerminal() {
    HANDLE hIn = GetStdHandle(STD_INPUT_HANDLE);
    g_consoleState.saved = GetConsoleMode(hIn, &g_consoleState.mode);
    SetConsoleMode(hIn, g_consoleState.mode & ~(ENABLE_LINE_INPUT | ENABLE_ECHO_INPUT));
}

void InputHandler::restoreTerminal() {
    if (g_consoleState.saved) {
        SetConsoleMode(GetStdHandle(STD_INPUT_HANDLE), g_consoleState.mode);
    }
}
...
InputHandler::InputHandler() : running_(true) {
    setupTerminal();
}
